...
void * rxthread( void * v )
{
	...
	while(1)
	{
		...
		int i;
		short * s = (short*)&RXbuf[26];
		static FILE * f;
		if( !f ) f = fopen( "/tmp/log.dat", "wb" );
		for( i = 0; i < RXbuf[25]; i++ )
		{
			fwrite( s+i, 2, 1,f );
		}
...
void * txthread( void * v )
{
	...
	while(1)
	{
		...
		for( i = 0; i < 8; i++ )
		{
			...
			rk = HSVtoHEX( i * 0.12 + frame* .001, (sat<1)?sat:1, 1.0 );
...
#endif
		usleep(10000); //Need a delay to allow LEDs to update.
		frame++;
