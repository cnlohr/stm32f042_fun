...
#define ADC_CIRC_BUF_SIZE 64

uint32_t adc_head, adc_tail;
uint16_t ADCBuffer[ADC_CIRC_BUF_SIZE];
int sampset;
uint16_t cumulative;

uint8_t pop_adc( uint16_t * v )
{
	uint16_t tail = adc_tail;
	if( tail == adc_head ) return 0;
	*v = ADCBuffer[tail];
	adc_tail = (tail+1) & (ADC_CIRC_BUF_SIZE-1);
	return 1;
}
...
//	GPIO_InitStructure.GPIO_Pin = 1<<0; //PB0
//	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AN;
//	GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_NOPULL ;
//	GPIO_Init(GPIOB, &GPIO_InitStructure);

	ConfigureGPIO( 0x10, INOUT_IN | PUPD_UP );
	GPIOAF( 0x10, 3 );
...
//	GPIO_Init(GPIOB, &GPIO_InitStructure);
	...
	ADC_ChannelConfig( ADC1, 8, /*ADC_SampleTime_13_5Cycles*/ADC_SampleTime_13_5Cycles);
...
void __attribute__ ((interrupt("IRQ"))) ADC1_IRQHandler(void)
{
	...
	if( ADC1->ISR & ADC_IT_EOSEQ )
	{
		...
		if( sampset == 16 )
		{
			...
			head = (head+1)&(ADC_CIRC_BUF_SIZE-1);
			if( head != adc_tail )
			{
				ADCBuffer[head] = cumulative;
				adc_head = head;
			}
			cumulative = 0;
			sampset = 0;
